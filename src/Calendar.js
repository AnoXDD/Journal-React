...
type Props = {|
  ...
  +data: Data,
  +debug?: boolean,
  +onBlockClick: (scrollTop: ?number) => void,
  +version: number,
  +year: number,
|};
...
export default class Calendar extends React.Component<Props, State> {
  ...
  generateFirstPadding(): React.Node {
  ...
  generateLastPadding(): React.Node {
    return (
      <span className="last-padding"></span>
    )
  }

  handleClick(month: number, day: number): void {
  ...
  render(): React.Node {
    ...
    let maxHeight = (new Date(this.props.year || new Date().getFullYear(),
      0).getDay() === 6 && this.MONTH_DAY[1] === 29) ? 1150 : 1130;

    return (
      <div
        className="Calendar"
        style={{maxHeight: `${maxHeight}px`}}>
        <NoScrollArea padding="10px" backgroundColor="#424242">
            ...
            <div className="day-blocks">
              ...
              {yearBlocks.map((month, im) =>
                monthBlocks.map((day, id) => {
                    if (new Date(this.props.year,
                        im,
                        id + 2) > this.now || id >= this.MONTH_DAY[im]) {
                      return "";
                    }
                    return (
                      <div key={`calendar-${month}-${day}`}
                           className={`day-block accent-${Math.min(9,
                             this.state.squares[im][id] || 0)}${id === 0 ? ` month-first month-${im} ` : " "}${this.state.triangles[im][id] != null ? "triangle " : " "}${id >= this.MONTH_DAY[im] - 7 ? " last-seven" : " "}${id === this.MONTH_DAY[im] - 1 ? " last-one" : " "}`}
                      >
